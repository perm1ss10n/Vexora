...
// PubSubClient должен быть сконструирован с конкретным Arduino Client (WiFiClient, TinyGsmClient, ...).
// Храним указатель на singleton PubSubClient, созданный в init().
...
static const char* resolveLwtTopic(const MqttConfig& cfg) {
    ...
    if (isNonEmpty(cfg.lwtTopic)) {
        return cfg.lwtTopic;
    }

    // 2) If deviceId is known, build: v1/dev/<deviceId>/lwt
    if (!isNonEmpty(cfg.deviceId)) {
        return nullptr;
    }
...
static void onMessage(char* topic, uint8_t* payload, unsigned int len) {
    ...
    if (g_msgCb) {
        g_msgCb(t, payload, len);
        return;
    }

    // Fallback: minimal log if no callback is registered
    char buf[96];
...
static void subscribeTopics()
{
    ...
    if (vx_build_topic(topic, sizeof(topic), g_cfg->deviceId, VX_T_CMD)) {
        g_client->subscribe(topic, 1);
    }
    if (vx_build_topic(topic, sizeof(topic), g_cfg->deviceId, VX_T_CFG)) {
        g_client->subscribe(topic, 1);
    }
    if (vx_build_topic(topic, sizeof(topic), g_cfg->deviceId, VX_T_OTA)) {
        g_client->subscribe(topic, 1);
    }
}
...
{
    static PubSubClient client(netClient); // lifetime = весь процесс
    g_client = &client;
    g_cfg = &cfg;
    ...
    // Keepalive в секундах (обычно 15–60)
    g_client->setKeepAlive(30);

    // Callback на входящие
...
void MqttClient::loop()
{
    ...
    if (g_nextRetryMs != 0 && now < g_nextRetryMs)
    {
        return;
    }
    ...
    // Планируем следующую попытку с экспоненциальным backoff
    g_reconnectCount++;

    g_nextRetryMs = now + g_backoffMs;

    // Удваиваем, но ограничиваем максимумом
    ...
    char buf[96];
    snprintf(buf, sizeof(buf), "reconnect in %ums (count=%lu)", (unsigned)g_backoffMs, (unsigned long)g_reconnectCount);
    LOGW("MQTT", buf);
}
...
void MqttClient::setDeviceId(const char* deviceId) {
    if (!g_cfg) return;
    const_cast<MqttConfig*>(g_cfg)->deviceId = deviceId;
}